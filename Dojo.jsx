...
function Practice({ store, logActivity, back, shell, seed, consumeSeed }) {
  ...
  async function openBrief(c) {
    ...
    const sys = "Given a person and the exact setting, produce a pre-approach brief for someone about to walk up cold.\n- observable: 3-4 concrete things a stranger could genuinely notice in this moment and use to open. Short phrases.\n- interests: 3-5 real hobbies or likes that are NOT visible and would only surface through conversation.\n- approach: one sentence on the kind of opener that fits this specific person.\nRespond ONLY with JSON: {\"observable\":[],\"interests\":[],\"approach\":\"\"}";
  ...
  async function send() {
    ...
    const system = char.persona + moodLine + recoveryLine + "\n\nThis is private conversation practice; the human is rehearsing how to approach people. They are opening cold — you have not spoken before; their first message is an unprompted approach in the setting described. React exactly like the real person you are: a cold approach can land well or badly depending entirely on how it's done. Never break character, never coach, never explain yourself. Reply the way you'd actually respond: short, natural. Move at a realistic pace; do not hand out warmth, your number, or a meeting unless it has genuinely been earned. If the opener is generic, needy, intense, or creepy, react accordingly and let interest fall. If interest gets very low you may end the conversation.\n\nRespond ONLY with JSON: {\"reply\":\"<what you say>\",\"interest\":<integer 0-100>,\"aside\":\"<your honest unspoken thought, first person, one short sentence>\"}";
  ...
  if (stage === "lobby") {
    ...
    return (
      <div style={shell}><div className="mx-auto w-full" style={{ maxWidth: 720, padding: "18px 18px 64px" }}>
        ...
        <div className="flex" style={{ gap: 8, marginTop: 18, flexWrap: "wrap" }}>{["All", "Social", "Real estate"].map((t) => { const a = filter === t; return <button key={t} onClick={() => setFilter(t)} style={{ fontSize: 12.5, color: a ? C.youInk : C.soft, background: a ? C.you : "transparent", border: `1px solid ${a ? C.you : C.line}`, borderRadius: 999, padding: "6px 14px", cursor: "pointer" }}>{t}</button>; })}</div>
        <div style={{ marginTop: 16, display: "grid", gap: 12 }}>
          {list.map((c) => (
      ...
      <div style={shell}><div className="mx-auto w-full" style={{ maxWidth: 720, padding: "18px 18px 64px" }}>
        ...
        {brief && (
          <div style={{ marginTop: 22, display: "grid", gap: 14 }}>
            ...
            <div style={{ color: C.faint, fontSize: 12.5, lineHeight: 1.55, fontStyle: "italic" }}>Before you walk over: one slow breath. Lead with the observable. You don't need a perfect line — you need a real one.</div>
          </div>
        )}
        <button onClick={() => start(briefChar)} disabled={briefLoading} style={{ marginTop: 22, width: "100%", background: briefLoading ? C.raised : C.you, color: briefLoading ? C.faint : C.youInk, border: "none", borderRadius: 12, padding: "14px", cursor: briefLoading ? "default" : "pointer", fontSize: 15, fontWeight: 600 }}>Walk over and open</button>
      </div></div>
    );
  }
    ...
    <div style={{ ...shell, height: "100dvh", minHeight: 0, display: "flex", flexDirection: "column" }}>
      ...
      <div ref={scrollRef} className="w-full" style={{ flex: 1, overflowY: "auto", minHeight: 0, WebkitOverflowScrolling: "touch" }}>
        <div className="mx-auto w-full" style={{ maxWidth: 720, padding: "18px 14px" }}>
          {messages.length === 0 && <div style={{ textAlign: "center", marginTop: 32 }}><Avatar name={char.name} size={64} /><div style={{ fontFamily: DISPLAY, fontSize: 18, color: C.soft, marginTop: 12 }}>{char.scene}</div><div style={{ color: C.faint, fontSize: 13.5, lineHeight: 1.6, marginTop: 6, maxWidth: 360, marginLeft: "auto", marginRight: "auto" }}>{char.name} is right there and hasn't noticed you. Your move — open it.</div></div>}
          {messages.map((m, i) => (
